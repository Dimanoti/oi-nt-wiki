...
更接近的，有 $\displaystyle \pi(n) \sim \mathrm{Li}(n)=\int_2^n \frac{1}{\ln(t)}\mathrm{d}t$。
由 [Littlewood定理](/oi-nt-wiki/Littlewood定理)，存在无穷个 $\pi(n)<\mathrm{Li}(n)$，也存在无穷个 $\pi(n)>\mathrm{Li}(n)$。
